 ...
 * the License.
 */
package org.palaso.languageforge.client.lex.controls.gwtcanvas;
  ...
  private native ImageElement prepareImage(String url)/*-{
    ...
    img.onload = function() {
      if(!img.__isLoaded) {
        ...
        img.__isLoaded = true;       
        __this.@org.palaso.languageforge.client.lex.controls.gwtcanvas.ImageLoader::incrementLoadedImages()();
        img.onload = null;
        
        // we call this function each time onload fires
        // It will see if we are ready to invoke the callback
        __this.@org.palaso.languageforge.client.lex.controls.gwtcanvas.ImageLoader::dispatchIfComplete()();   
      } else {
