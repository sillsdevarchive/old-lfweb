<?php
#
# Copyright 2010 Manolo Carrasco Mo�ino. (manolo at apache/org)
...
$uploaddir = PHP_UPLOAD_PATH . session_id() . "/";
...
if($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	if ($status['total']) {
		...
		if ( $total > PHP_UPLOAD_MAX_FILE_SIZE)
		{
			writeResponse("<error>" . "The request was rejected because its size: ".  round($total/1024) . " KB, exceeds the maximum: " .  round(PHP_UPLOAD_MAX_FILE_SIZE/1024) . " KB" . "</error>", 0);
			return;
		}
		...
		writeResponse("<error>File can not be uploaded.</error>", 0);
	}
