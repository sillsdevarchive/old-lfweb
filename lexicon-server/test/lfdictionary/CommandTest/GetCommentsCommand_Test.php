...
class TestOfGetCommentsCommand extends UnitTestCase {
	...
	function testGetComments() {
		$chorusNotesFilePath= TEST_PATH. "data/Test.lift.ChorusNotes";
		$commandAll_Not_Recent = new \commands\GetCommentsCommand($chorusNotesFilePath, "","", 0,999,false);
		$result = json_encode($commandAll_Not_Recent->execute()->encode());
		$this->assertEqual($this->FINAL_RESULT_ALL, $result);
		
		$chorusNotesFilePath= TEST_PATH. "data/Test.lift.ChorusNotes";
		$commandAll_Question_Not_Recent = new \commands\GetCommentsCommand($chorusNotesFilePath, "","question", 0,999,false);
		$result = json_encode($commandAll_Question_Not_Recent->execute()->encode());
		$this->assertEqual($this->FINAL_RESULT_ALL_QUESTION, $result);
		
		$chorusNotesFilePath= TEST_PATH. "data/Test.lift.ChorusNotes";
		$commandAll_mergeConflict_Not_Recent = new \commands\GetCommentsCommand($chorusNotesFilePath, "","mergeConflict", 0,999,false);
		...
		$commandFirst_Not_Recent = new \commands\GetCommentsCommand($chorusNotesFilePath, "","", 0,1,false);
		$result = json_encode($commandFirst_Not_Recent->execute()->encode());
		$this->assertEqual($this->FINAL_RESULT_FIRST, $result);
		
		$commandRecent = new \commands\GetCommentsCommand($chorusNotesFilePath, "","", 0,1,true);
		$result = json_encode($commandRecent->execute()->encode());
		$this->assertEqual($this->FINAL_RESULT_RECENT, $result);
		
		$commandAll_Closed = new \commands\GetCommentsCommand($chorusNotesFilePath, "closed","", 0,999,false);
		$result = json_encode($commandAll_Closed->execute()->encode());
		$this->assertEqual($this->FINAL_RESULT_CLOSED, $result);
		
		$commandAll_Unclosed = new \commands\GetCommentsCommand($chorusNotesFilePath, "unclosed","", 0,999,false);
