...
// scrollable body
var VSA_default_GwtBasePath = "";

function VSA_initScrollbars(gwtbasepath) {
	if (!document.body.children)
		return;
	var scrollElements = VSA_getElements("vscrollable", "DIV", document,
			"class");
	VSA_default_GwtBasePath = gwtbasepath;
...
function VSA_createScrollBar() {
	...
	if (this.enableScrollbar) {
		...
		this.scrollup.innerHTML = '<img src="' + VSA_default_GwtBasePath + '/'
				+ this.imagesPath + '/' + this.btnUpImage + '" border="0"/>';
		...
		this.scrolldown.innerHTML = '<img src="' + VSA_default_GwtBasePath
				+ '/' + this.imagesPath + '/' + this.btnDownImage
				+ '" border="0"/>';
		...
		if (this.enableScrollbarSlider) {
			...
			if (VSA_touchFlag) {
				this.scrollslider.addEventListener("touchstart", touchHandler,
						true);
			}
			if (document.all)
